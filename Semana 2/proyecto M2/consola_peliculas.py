...
def ejecutar_encontrar_pelicula_mas_larga(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    """
    nombre = mod.encontrar_pelicula_mas_larga(p1, p2, p3, p4, p5)["nombre"]
    duracion = mod.encontrar_pelicula_mas_larga(p1, p2, p3, p4, p5)["duracion"]
    print(f"La pelicula mas larga es {nombre} con {duracion} minutos de duracion")
   # TODO: Completar

def ejecutar_consultar_duracion_promedio_peliculas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    print(f"La duracion promedio de las peliculas es: {mod.duracion_promedio_peliculas(p1, p2, p3, p4, p5)}")
   # TODO: Completar
...
def ejecutar_encontrar_estrenos(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    print(f"Las peliculas estrenadas en una fecha posterior a {anio} son: {mod.encontrar_estrenos(p1, p2, p3, p4, p5, anio)}")
   # TODO: Completar
...
def ejecutar_cuantas_peliculas_18_mas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    Parametros:
        ...
        p5 (dict): Diccionario que contiene la informacion de la pelicula 5.
    """
    print(f"El numero de peliculas 18+ es: {mod.cuantas_peliculas_18_mas(p1, p2, p3, p4, p5)}")
   # TODO: Completar
...
def ejecutar_reagendar_pelicula(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    if pelicula is None:
        ...
        nueva_hora = int(input("Ingrese la nueva hora de la  pelicula: "))
        nuevo_dia = input("Ingrese eñ nuevo dia de la pelicula: ")
        control_horario = input("¿Desea tener control horario? [s/n]: ")
        if str.lower(control_horario) == "s":
            op = True
        else:
            op = False
        if mod.reagendar_pelicula(pelicula, nueva_hora, nuevo_dia, op, p1, p2, p3, p4, p5):
            print("¡La pelicula ha sido reagendada con éxito!")
        else:
...
def ejecutar_decidir_invitar(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict)->None:
    ...
    if pelicula is None:
        ...
        auto_padres = input("¿El invitado tiene autorizacion de los padres? [s/n]: ")
        if str.lower(auto_padres) == "s":
            op = True
        else:
            op = False
        
        if mod.decidir_invitar(pelicula, edad_invitado, op):
            print("Se puede invitar a la persona")
        else:
            print("NO se puede invitar a la pesona")
   # TODO: Completar
